var System  = {
    ...
    create_bootstarp_dialog : function(json) {
        System.create_bootstarp_confirm(json);
    },
    /**
     * 2:confirm
     * 
    ...
    create_bootstarp_confirm : function(json) {
        ...
        var height = !json.height ? '350' : Number(json.height);
        ...
        var div1 = $('<div>', {
            ...
            'aria-hidden' : 'true',
            'style' : 'overflow:auto;margin-top: 200px'
        });
        var div2 = $('<div>', {
            'class' : 'modal-dialog',
            'role' : 'document',
        ...
        // 防止重复创建数组
        if (!System.fun_array) {
            System.fun_array = new Array();
        }
        // 标记:为每个按钮添加标记,点击时根据标记调用数组中的函数
        var i = 0;
        for ( var name in button) {
            System.fun_array.push(button[name]);
            var but = $('<button>', {
                'type' : 'button',
                ...
                'html' : name,
                'click' : function() {
                    System.fun_array[Number($(this).attr('idx'))]();
                    // 点击按钮后清空数据,防止重复添加数据,因为每次点击,都会向数组中添加函数
                    System.fun_array.length = 0;
